...
def _month_file_path(dt=None) -> Path:
    dt = dt or datetime.now(timezone.utc)
    return DATA_DIR / f"{dt.strftime('%Y-%m')}.jsonl"
...
def append_items(items: list) -> list:
    """Appends items not already present (by id) to this month's file.
    Returns only the items that were actually new, i.e. today's fresh
    finds -- reruns of the same sweep won't re-print old hits."""
    if not items:
        return []
